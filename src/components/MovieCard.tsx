          ...
          return (
            <div key={index} className="flex flex-col w-full pb-2">
              <div className="flex flex-col items-start w-full tracking-wider">
                  ...
                  {isHovered === index && (
                    <div className="absolute inset-0 flex flex-col justify-center px-4 sm:px-10 py-8 sm:py-36 rounded-md bg-black bg-opacity-40">
                      <div className="flex flex-col w-full">
                        <Link className="gap-1.5 self-stretch px-3 sm:px-5 py-2 sm:py-3 w-full text-white whitespace-nowrap rounded-md border border-white border-solid text-center text-sm" to={`/details/${movie.id}`}>
                          Details
                        </Link>
                        <Link className="gap-1.5 self-stretch px-3 sm:px-5 py-2 sm:py-3 mt-3 w-full bg-blue-700 rounded-md text-slate-50 text-center text-sm" to={`/details/${movie.id}`}>
                          Buy Ticket
                        </Link>
                      </div>
                    </div>
                ...
                <h3 className="mt-5 text-lg font-bold tracking-wider leading-none text-slate-900">{movie.title}</h3>
                <div className="flex flex-wrap gap-2 items-start mt-3 sm:mt-5 text-sm leading-8 text-center whitespace-nowrap text-slate-700">
                  {genres.map((genre, idx) => (
                    <span key={idx} className="gap-2.5 self-stretch px-3 sm:px-5 rounded-3xl bg-slate-400 bg-opacity-10 h-[31px]">
                      {genre}
                    </span>
                  ))}
                </div>
